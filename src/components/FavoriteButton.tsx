...
interface Props {
  ...
  onToggle?: (action: 'added' | 'removed', id: number | string) => void; // Reporta la acción exacta
...
export default function FavoriteButton({ mangaId, isInitiallyFavorite = false, onToggle, displayMode = 'icon' }: Props) {
  ...
  const handleToggleFavorite = async (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation(); // 🚀 CRÍTICO: Evita que se abra el modal de la tarjeta
    
    setIsProcessing(true);
    setStatus('idle');
    let actionDone: 'added' | 'removed' = 'added';
    ...
    try {
      ...
      setStatus('success');
      if (onToggle) onToggle(actionDone, mangaId); // Enviamos qué pasó y a quién
  ...
  // MODO TEXTO (Para SingleMangaDetail)
      ...
      <div className="absolute top-4 right-4 animate-in zoom-in duration-300 z-20">
        <div className="bg-emerald-100/90 backdrop-blur-md border border-emerald-200 px-4 py-2 rounded-3xl shadow-sm flex items-center gap-2">
          ...
          <button 
            ...
            className="cursor-pointer text-emerald-400 hover:text-rose-500 transition-colors ml-1 p-0.5 outline-none"
          >
            {isProcessing ? (
  ...
  return (
    ...
    >
      {isProcessing ? (
        ...
        /* 🗑️ ICONO DE ELIMINAR (Solo en la Bóveda) */
        <svg className="w-5 h-5 text-rose-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
        </svg>
      ) : (
        /* ❤️ ICONO DE CORAZÓN (En Top/Búsquedas) */
        <svg className={`w-5 h-5 transition-colors ${isFavorite ? 'text-rose-500' : 'text-slate-400'}`} fill="currentColor" viewBox="0 0 24 24">
